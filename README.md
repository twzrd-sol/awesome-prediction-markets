...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) — Trust scoring for AI agents on Solana. `score_agent(wallet)` and `preflight_check(wallet)` verify agent wallet identity before routing capital signals or accessing premium market data. `get_trust_receipt(wallet)` issues a signed x402 micropayment receipt. MCP endpoint: `https://intel.twzrd.xyz/mcp`.
  ```json
